...
public class LecturePageController {
     ...
     // Fi west el initialize() walla initFilm/initEpisode
        ...
        if (addToListButton != null) {
            ...
            MyListManager.getInstance().addListener((filmId, serieId) ->
                Platform.runLater(() -> updateAddButton())
            );
        }
        if (btnBack != null) {
    ...
    private void updateAddButton() {
        if (currentItem == null || addToListButton == null) return;
        ...
        if (alreadyAdded) {
            ...
            addToListButton.setStyle(
                "-fx-background-color: #00aaff; -fx-font-weight: bold; " +
                "-fx-text-fill: white; -fx-background-radius: 25; -fx-cursor: hand;"
            );
        } else {
